...
- **Modal scroll lock (iOS):** `overflow: hidden` on body does NOT lock scroll on iOS Safari — the page scrolls behind the modal. The fix (in `setPageScrollLock`) pins `body { position: fixed; top: -<scrollY>px; width: 100% }` while open and restores `window.scrollTo(0, scrollY)` on close. Don't revert to overflow:hidden.
- **Modal sizing (mobile):** the modal overlay uses `height: 100dvh` (visible viewport, NOT lvh) and is itself the scroll container (`overflow-y: auto; align-items: start`); the panel uses `max-height: none; margin: auto 0` so it centers when short and pins-to-top-scrollable when tall. Using `lvh` made the panel taller than the visible screen, clipping the title/close button off the top. The `.modal-backdrop` is `position: fixed; inset: 0` so it dims the full physical screen (no undimmed strips). `openModal` resets `modal.scrollTop = 0` so it always opens at the title.
